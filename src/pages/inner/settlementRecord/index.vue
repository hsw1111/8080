  ...
  <div>
    ...
    <div id="settleRed_content">
      <div id="settleRed_header">
        ...
        <h3>
          ...
          <span>元</span>
          <p>累计已结算金额：</p>
          <span>{{alreadyWidthDrawMoney}}</span>
          <span>元</span>
        </h3>
      </div>
    </div>
    ...
    <div id="settleRed_table">
      <el-table :data="tableData" v-loading="loading2" element-loading-text="拼命加载中" style="width: 100%" @cell-click='goDetail' :empty-text="emptyText">
        ...
        <el-table-column prop="applyMoney" label="结算金额" min-width="100">
        </el-table-column>
        <el-table-column prop="applyTimeStr" label="申请时间" min-width="120">
        </el-table-column>
...
#settleRed_header button.canWidthDrawMoney {
  width: 120px;
  height: 40px;
...
#settleRed_header button.canWidthDrawMoney:hover {
  background: rgba(255, 153, 0, 1);
  cursor: pointer;
}

#settleRed_header button.canWidthDrawMoney+span {
  color: #aaa;
  margin: 6px 0 0 5px;
  font-style: italic;
}

#settleRed_header button.is-disabled {
  float: right;
  margin: 10px 0 0 10px;
  width: 120px;
  height: 40px;
      ...
      .end((err, res) => {
        if (err) {
          ...
          if (code != -1) {
            ...
            this.canWidthDrawMoney =  this.cityPartner.canWidthDrawMoney
            this.alreadyWidthDrawMoney = this.cityPartner.alreadyWidthDrawMoney
            this.currentMonthIncome = this.cityPartner.currentMonthIncome
          }else{
            this.pageShow = false
            this.emptyText = ' 暂无数据'
            var newArr = []
